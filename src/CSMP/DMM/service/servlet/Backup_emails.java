...
public class Backup_emails extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private String Replace_dbTableSechema = "replace into opportunities(id,deleted,SME_ID,date_entered,date_modified,"+
					"assigned_user_id,modified_user_id,created_by,date_sent,date_received,name,type,reply_to_status,"+
					"parent_type,parent_id,email_from,email_to,contents)" ;
	...
	protected void doPost(HttpServletRequest request,
		...
		assigned_user_id = URLDecoder.decode(request.getParameter("date_sent"),"UTF-8");
		date_sent = URLDecoder.decode(request.getParameter("date_sent"),"UTF-8");
		date_received = URLDecoder.decode(request.getParameter("date_received"),"UTF-8");
		name = URLDecoder.decode(request.getParameter("name"),"UTF-8");
		type = URLDecoder.decode(request.getParameter("type"),"UTF-8");
		reply_to_status = URLDecoder.decode(request.getParameter("reply_to_status"),"UTF-8");
		...
		email_from = URLDecoder.decode(request.getParameter("email_from"),"UTF-8");
		email_to = URLDecoder.decode(request.getParameter("email_to"),"UTF-8");
...
//		token_verification tt = new token_verification();
	   ...
	   // if( varify == 1){
		...
		try {
			connection = services.getInstance(CSMP_DMM_API.backup_emails_v1,"emails");
			if (connection != null) {

				 //original SQL syntax
				 BasicDBObject doc = new BasicDBObject("schema", Replace_dbTableSechema+"values("+id+","+deleted+","+SME_ID+","+date_entered+","+date_modified+","+assigned_user_id+","+modified_user_id+","+created_by+","+date_sent+","+date_received+","+name+","+type+","+reply_to_status+","+parent_type+","+parent_id+","+email_from+","+email_to+","+contents+");");
