...
@WebServlet("/Insert_API_vendors")
public class Backup_vendors extends HttpServlet {
	...
	private String Replace_dbTableSechema = "replace into opportunities(id,deleted,SME_ID,date_entered,date_modified,modified_user_id,"+
					"created_by,description,assigned_user_id,name,phone,email,website,glacct,category,street,city,state,country,postalcode,pobox) ";
	...
	protected void doPost(HttpServletRequest request,
		...
		phone = URLDecoder.decode(request.getParameter("phone"),"UTF-8");
		...
		street = URLDecoder.decode(request.getParameter("street"),"UTF-8");
		city = URLDecoder.decode(request.getParameter("city"),"UTF-8");
		state = URLDecoder.decode(request.getParameter("state"),"UTF-8");
		country = URLDecoder.decode(request.getParameter("country"),"UTF-8");
		postalcode = URLDecoder.decode(request.getParameter("postalcode"),"UTF-8");
		pobox = URLDecoder.decode(request.getParameter("pobox"),"UTF-8");
...
//		token_verification tt = new token_verification();
	   ...
	   // if( varify == 1){
		...
		try {
			connection = services.getInstance(CSMP_DMM_API.backup_vendors_v1,"vendors");
			if (connection != null) {

				 //original SQL syntax
				 BasicDBObject doc = new BasicDBObject("schema", Replace_dbTableSechema+"values("+id+","+deleted+","+SME_ID+","+date_entered+","+date_modified+","+modified_user_id+","+created_by+","+description+","+assigned_user_id+","+name+","+phone+","+email+","+website+","+glacct+","+category+","+street+","+city+","+state+","+country+","+postalcode+","+pobox+");");
